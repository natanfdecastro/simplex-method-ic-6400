...
class DualMethod:
    ...
    def solve(self):
        ...
        while True:
            ...
            if all(fi >= 0 if self.max_min_operation_to_use == "max" else fi <= 0 for fi in self.w_function_values[:-1]):  # if the plan is optimal
                break  # finish the operation

            column = (np.argmin if self.max_min_operation_to_use == "max" else np.argmax)(self.w_function_values[:-1])  # we get the resolving column
